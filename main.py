...
st.write(
    "Now we have our data defined, we'll use a selection of models and see which performs best out the box! For this we'll need classificatgion algorithms, let's see how they perform!")
...
def modeldev (i, j, k, l, m):
    ...
    st.write((classification_report(m, y_pred)))


modeldev(i, X_train, y_train, X_test, y_test)

st.write(
    "Now we have our models outputting metrics we can see that the KNeighbours classifier is performing best out the box. Let's try and optimise its performance using hyperparameter tuning!")
st.write(
